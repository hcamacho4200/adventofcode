...
fn entry(input: &str, name: &str) -> i32 {
    ...
    for (row, line) in l.iter().enumerate() {
        let mut number_row: Vec<Number> = Vec::new();
        let mut symbol_row: Vec<Symbol> = Vec::new();

        for (line_pos, sym_or_num) in line {
            for (hson, pos) in handle_symbol_or_number(sym_or_num.to_string()) {
                // println!("{} {} {} {:?}", row, line_pos, pos, hson);

                match hson {
                    Either::Left(symbol) => {
                        symbol_row.push(Symbol {
                            line_pos: line_pos.clone() as i32 + pos,
    ...
    for (symbol_row, symbols) in symbol_vec.iter_mut().enumerate() {
        for symbol in symbols.iter_mut() {
            ...
            println!("{} {} {:?}", symbol_row, symbol.line_pos, symbol.symbol);

            // check adjacent numbers left and right
            for number in number_vec[symbol_row.clone()].iter_mut() {
                if number.line_pos == &symbol.line_pos - 1 {
                    println!("{} left", number.value);
                    number.seen = true;
                    symbol.numbers.push(number.clone());
                } else if &number.line_pos - (number.value.to_string().len() as i32 -1) as i32 == &symbol.line_pos + 1 {
                    println!("{} right", number.value);
                    number.seen = true;
                    symbol.numbers.push(number.clone());
                }
            }
            // check adjacent numbers up and down
            for number in number_vec[(&symbol_row - 1).clone()].iter_mut() {
                if symbol.line_pos <= number.line_pos && symbol.line_pos >= &number.line_pos - (number.value.to_string().len() as i32 - 1) {
                    println!("{} above", number.value);
                    number.seen = true;
                    symbol.numbers.push(number.clone());
                } else if &symbol.line_pos + 1 == &number.line_pos - (number.value.to_string().len() as i32 - 1) {
                    println!("{} up and right", number.value);
                    number.seen = true;
                    symbol.numbers.push(number.clone());
                } else if &symbol.line_pos - 1 == number.clone().line_pos as i32  {
                    println!("{} up and left", number.value);
                    number.seen = true;
                    symbol.numbers.push(number.clone());
                }
            }
            for number in number_vec[(&symbol_row + 1).clone()].iter_mut() {
                if symbol.line_pos <= number.line_pos && symbol.line_pos >= &number.line_pos - (number.value.to_string().len() as i32 - 1) {
                    println!("{} {} {} below", number.value, number.line_pos, &number.line_pos - (number.value.to_string().len() as i32 - 1));
                    number.seen = true;
                    symbol.numbers.push(number.clone());
                } else if &symbol.line_pos + 1 == &number.line_pos - (number.value.to_string().len() as i32 - 1) {
                    println!("{} down and right", number.value);
                    number.seen = true;
                    symbol.numbers.push(number.clone());
            } else if &symbol.line_pos - 1 == number.clone().line_pos as i32  {
                    println!("{} down and left", number.value);
                    number.seen = true;
                    symbol.numbers.push(number.clone());
                }
            }
    ...
    let mut total = 0;
    for symbol_line in symbol_vec.iter() {
        for symbol in symbol_line.iter() {
            println!("{:?}", symbol);
            if symbol.symbol == "*" {
                if symbol.numbers.len() == 2 {
                    let ratio = &symbol.numbers[0].value * &symbol.numbers[1].value;
                    total += ratio
                }
            }
        }
...
fn handle_symbol_or_number(symbol_or_number: String) -> Vec<(Either<String, i32>, i32)> {
    let mut is_num = false;
    let mut collect_number = "".to_string();
    let mut results: Vec<(Either<String, i32>, i32)> = Vec::new();
    for (i, ch) in symbol_or_number.chars().enumerate() {
        match is_num {
            false => {
                if ch.is_numeric() {
                    is_num = true;
                    collect_number.push(ch);
                } else {
                    results.push((Either::Left(ch.to_string()), i as i32));
                }
            },
            true => {
                if ch.is_numeric() {
                    ...
                    results.push((Either::Right(collect_number.parse::<i32>().unwrap()), (i.clone() -1) as i32));
                    results.push((Either::Left(ch.to_string()), i as i32));
                    is_num = false;
                    collect_number = "".to_string();
                }
            }
        }
    }
    ...
    if is_num {
        results.push((Either::Right(collect_number.parse::<i32>().unwrap()), (symbol_or_number.len() -1) as i32));
    }
...
use either::*;
#[test]
pub fn test_aoc_2023_03_2_handle_symbol_or_number() {
    let actual = handle_symbol_or_number("*981*582*".to_string());
    println!("{:?}", actual);

    assert_eq!(actual[0], (Either::Left("*".to_string()), 0));
    assert_eq!(actual[1], (Either::Right(981), 3));
    assert_eq!(actual[2], (Either::Left("*".to_string()), 4));
    assert_eq!(actual[3], (Either::Right(582), 7));
    assert_eq!(actual[4], (Either::Left("*".to_string()), 8));

    let actual = handle_symbol_or_number("981".to_string());
    println!("{:?}", actual);

    assert_eq!(actual[0], (Either::Right(981), 2));

    let actual = handle_symbol_or_number("**42*?".to_string());
    println!("{:?}", actual);

    assert_eq!(actual[0], (Either::Left("*".to_string()), 0));
    assert_eq!(actual[1], (Either::Left("*".to_string()), 1));
    assert_eq!(actual[2], (Either::Right(42), 3));
    assert_eq!(actual[3], (Either::Left("*".to_string()), 4));
    assert_eq!(actual[4], (Either::Left("?".to_string()), 5));

}

#[test]
#[named]
pub fn test_aoc_2023_03_2_entry() {
    let input = "467..114..
...*......
..35..633.
......#...
...
#[test]
#[named]
pub fn test_aoc_2023_03_2() {
    let input = ".....613...................................439............498.........................438......617....343.............942...................
