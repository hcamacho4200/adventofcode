...
fn entry(input: &str, name: &str) -> i32 {
    ...
    let mut good_games_total = 0;

    let mut color_limits: HashMap<String, i32> = HashMap::new();
    color_limits.insert("red".to_string(), 12);
    color_limits.insert("green".to_string(), 13);
    color_limits.insert("blue".to_string(), 14);
    ...
    for line in input.lines() {
        ...
        for game_session in game_sessions {
            let sessions = &game_session[1..];

            for mut grab in sessions.split(",").into_iter() {
                let mut grab_split = grab.trim().split(" ").into_iter();
                let count = grab_split.next().unwrap().parse::<i32>().unwrap();
                let color = grab_split.next().unwrap();

                match color_limits.get_mut(color) {
                    Some(limit) => {
                        if count > *limit {
                            game_bad = true;
                            break;
        ...
        if !game_bad {
            good_games_total += game_number;
        }
    }
    println!("good_games_total = {}", good_games_total);
    good_games_total
}

#[test]
#[named]
pub fn test_aoc_2023_02_1() {
